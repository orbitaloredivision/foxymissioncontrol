#!/bin/bash
#
# GUI Dashboard Installer for Orange Pi (EXPERIMENTAL branch)
# https://github.com/orbitaloredivision/foxymissioncontrol
#
# Same as install-guidashboard.sh but clones and tracks the "experimental" branch.
#
...
# - PM2 process manager
#
# Usage: sudo ./install-guidashboard-experimental.sh [OPTIONS]
#
# Options:
...
print_banner() {
    if [ "$PLAIN_MODE" = true ]; then
        echo ""
        echo "GUI Dashboard Installer (experimental) - github.com/orbitaloredivision/foxymissioncontrol"
        echo ""
        return
    fi
    ...
    echo ""
    echo -e "${CYAN}+==================================================================+${NC}"
    echo -e "${CYAN}|${NC}${BOLD}${WHITE}     GUI Dashboard Installer for Orange Pi (experimental)     ${NC}${CYAN}|${NC}"
...
clone_repository() {
    ...
    if [ -d "$REPO_DIR/.git" ]; then
        ...
        if ! run_as_orangepi "cd $REPO_DIR && git rev-parse HEAD" > /dev/null 2>&1; then
            ...
            local remote_rev=$(run_as_orangepi "cd $REPO_DIR && git rev-parse origin/$REPO_BRANCH" 2>/dev/null || echo "unknown")
            ...
            if [ "$local_rev" = "$remote_rev" ]; then
                ...
                run_as_orangepi "cd $REPO_DIR && git reset --hard origin/$REPO_BRANCH" 2>&1 | while IFS= read -r line; do
                    echo "    $line"
                done
    ...
    sudo -u orangepi git clone --progress --branch "$REPO_BRANCH" "$REPO_URL" "$REPO_DIR" > "$logfile" 2>&1 &
    local clone_pid=$!
...
# Generated by install-guidashboard-experimental.sh
...
show_help() {
    echo ""
    echo -e "${CYAN}GUI Dashboard Installer for Orange Pi (experimental branch)${NC}"
    echo ""
    echo -e "${WHITE}Usage:${NC} sudo ./install-guidashboard-experimental.sh [OPTIONS]"
    echo ""
    echo -e "${WHITE}Branch:${NC} ${GREEN}$REPO_BRANCH${NC}"
    echo ""
    ...
    echo -e "  sudo ./install-guidashboard-experimental.sh              # Normal install"
    echo -e "  sudo ./install-guidashboard-experimental.sh -reinstall   # Clean reinstall"
    echo -e "  sudo ./install-guidashboard-experimental.sh -reinstall -y # Clean reinstall without prompts"
    echo -e "  sudo ./install-guidashboard-experimental.sh -uninstall   # Remove everything"
    echo ""
}
