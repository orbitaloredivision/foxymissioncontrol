...
export function SpeedoReadout({ speed = 0 }) {
...
export function Odometer({ dist = 0 }) {
  const { t } = useTranslation()
...
function SpeedometerGauge({ speed = 0 }) {
  const speedKmh = speed ?? 0
  const maxSpeed = 40
...
export function Speedometer({ speed, dist }) {
  return (
    <div className="speedometer">
      <SpeedometerGauge speed={speed} />
      <SpeedoReadout speed={speed} />
      <Odometer dist={dist} />
    </div>
  )
}

/** Digital speed + odometer only (no gauge SVG). */
export function SpeedometerSimple({ speed, dist }) {
  return (
    <div className="speedometer speedometer--simple">
      <SpeedoReadout speed={speed} />
      <Odometer dist={dist} />
    </div>
  )
